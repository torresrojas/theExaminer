...
int main(int argc, char *argv[]) 
{
  ...
  if (PQstatus(DATABASE) == CONNECTION_BAD)
  ...
  else
     {
      ...
      if (!gtk_builder_add_from_file (builder, ".interfaces/EX-O.glade", &error))
         {
          g_warning ("%s\n", error->message);
          g_error_free (error);
         }
...
void Connect_Widgets ()
{
  ...
  MenuPrincipal     = GTK_WIDGET (gtk_builder_get_object (builder, "MenuPrincipal"));
...
void Interface_Coloring ()
{
  ...
  gdk_rgba_parse (&color, MAIN_AREA);
  gtk_widget_override_background_color(window1,  GTK_STATE_NORMAL,   &color);
  
  gdk_rgba_parse (&color, decoration_color);
  gtk_widget_override_background_color(MenuPrincipal,         GTK_STATE_NORMAL, &color);
