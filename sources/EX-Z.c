...
#define N_BACKGROUNDS 13
struct {char * file; char * color;} Background [N_BACKGROUNDS]=
{{".imagenes/EX-BG-01.jpg","yellow"},
 {".imagenes/EX-BG-02.jpg","black"},
...
int main(int argc, char *argv[]) 
{
  ...
  if (PQstatus(DATABASE) == CONNECTION_BAD)
  ...
  else
     {
      ...
      if (!gtk_builder_add_from_file (builder, ".interfaces/EX-Z.glade", &error))
         {
          g_warning ("%s\n", error->message);
          g_error_free (error);
         }
      else
	 {
	  ...
	  /* Un poco de color...  */
          gdk_color_parse (decoration_color, &color);
          gtk_widget_modify_bg(EN_materias,     GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_autores,      GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_ejercicios,   GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_preguntas,    GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_esquemas,     GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_pre_examenes, GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_examenes,     GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_evaluaciones, GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(EN_respaldo,     GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(BN_refresh,      GTK_STATE_PRELIGHT, &color);

          gdk_color_parse ("blue", &color);
          gtk_widget_modify_bg(BN_refresh,   GTK_STATE_NORMAL, &color);

          gdk_color_parse ("red", &color);
          gtk_widget_modify_bg(FR_no_disponible, GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_materias,   GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_autores,    GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_ejercicios, GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_preguntas,  GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_esquemas,   GTK_STATE_NORMAL, &color);
          gtk_widget_modify_bg(FR_no_configurado,GTK_STATE_NORMAL, &color);
